...
export class RoamingPartners implements OnInit {
  ...
  pagedPartners = computed(() => {
    ...
    return this.partners().slice(p * ps, (p + 1) * ps);
  });
  totalPages = computed(() => Math.ceil(this.partners().length / this.pageSize));
